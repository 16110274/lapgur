                            ...
                            <li class="active">
                                ...
                                <ul class="collapse">
                                    ...
                                    <li><a href="dashmin.guru">Guru</a></li>
                                     ...
                                     <li  class="active"><a href="dashmin.siswa">Siswa</a></li>                       
                                </ul>
                            </li>
                            <li>
...
<?php
    $nomor = 1;
      $output = '';  
      $sql = "select * from siswa where id_siswa=$idsiswa"; 
...
?>
                                        ...
                                        <div class="form-group">
                                         <label for="example-text-input" class="col-form-label">ID Siswa : </label>
                                            <input name="ids" class="form-control" type="text" readonly="" value="<?php echo $row['id_siswa'];  ?>" id="example-text-input">                                            
                                            <label for="example-text-input" class="col-form-label">Nama : </label>
                                            <input name="nama" class="form-control" type="text" value="<?php echo $row['nama_siswa'];  ?>" id="example-text-input">                                            
                                            <label for="example-text-input" class="col-form-label">Jenis Kelamin : </label>
                                            <select name="jk" class="custome-select border-0 pr-3" >
                                                  <?php echo $newsiswa -> jksekarang($koneksi);
                                                  echo $newsiswa -> jklain($koneksi);   
                                              ?>  
                                            </select> <br>                                                                                       
                                            <label for="example-text-input" class="col-form-label">Kelas : </label>
                                            <select name="kelas" class="custome-select border-0 pr-3" >
                                                  <?php echo $newsiswa -> kelassekarang($koneksi);
                                                  echo $newsiswa -> kelaslain($koneksi);   
                                              ?>  
                                            </select>
                                        </div>
...
//mysqli_query($koneksi,"insert into kbm (id_siswa,id_guru,id_kelas,id_semester,id_tahunajar) values('$mapel','$guru','$kelas','$semester','$tahunajar')");
mysqli_query($koneksi,"UPDATE `siswa` SET `nama_siswa`='$nama', `jk_siswa`='$jk', `id_kelas`='$kelas' WHERE `id_siswa`='$ids'");
